...
async function saveScheduleDuration(obj) {
    try{
        return await knex('schedule_duration')
            .insert({
                hash_id: hash(obj, {algorithm: 'md5', encoding: 'base64'}),
...
async function saveLessonAndBreakDuration(obj) {
    try{
        return await knex('events_duration')
            .insert({
                lesson_duration: obj.lesson_duration,
                interval_break: obj.interval_break,
                schedule_id: knex('schedule_duration')
                    .where('hash_id', hash(obj, {algorithm: 'md5', encoding: 'base64'}))
                    .select('id'),
            });
    }catch (err) {
        return err;
...
async function saveSpeakers(obj) {
    try{
...
module.exports = {
    saveOrganizations, saveScheduleDuration, saveLessonAndBreakDuration,
